...
%}
%start S
%token DO ID NUM T_lt T_gt T_lteq T_gteq T_neq T_eqeq T_and T_or T_incr T_decr T_not T_eq WHILE INT CHAR FLOAT VOID H MAINTOK INCLUDE BREAK CONTINUE IF ELSE PRINTF STRING FOR T_ques T_colon
...
LOOPS
      ...
      | DO {lookup($1,@1.last_line,'K',NULL,NULL);}'{' LOOPBODY '}' WHILE {lookup($6,@6.last_line,'K',NULL,NULL);}'(' COND ')' ';'
      ;
...
DECLARATION
      : TYPE A 
    A : ID',' A {c=1;search_id($1,@1.last_line); lookup($1,@1.last_line,'I',"0",$1);}|ID {c=1;search_id($1,@1.last_line);lookup($1,@1.last_line,'I',"0",$1);};;


COND
      : COND RELOP FACTOR 
      | COND bin_boolop FACTOR
      | un_boolop '(' COND ')'
      | un_boolop FACTOR
      | FACTOR
      ;
...
ASSIGN_EXPR
      ...
      |TYPE ID T_eq EXP {c=1;search_id($2,@2.last_line); lookup($2,@1.last_line,'I',NULL,$1);lookup($3,@3.last_line,'O',NULL,NULL);update($2,@1.last_line,$4);} 
      ;
...
PL_MN
      : TERM 
      | PL_MN T_pl TERM {lookup($2,@1.last_line,'O',NULL,NULL);sprintf($$,"%d",atoi($1)+atoi($3));} //printf("%s\n",$$);}
      | PL_MN T_min TERM {lookup($2,@1.last_line,'O',NULL,NULL);sprintf($$,"%d",atoi($1)-atoi($3));} //printf("%s\n",$$);}
      ;
...
PRINT
      : PRINTF '(' STRING ')' {lookup($1,@1.last_line,'K',NULL,NULL);lookup($4,@1.last_line,'S',NULL,NULL);}
      ;
