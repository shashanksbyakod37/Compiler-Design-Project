...
%start S
%token ID NUM T_lt T_gt T_lteq T_gteq T_neq T_eqeq T_and T_or T_incr T_decr T_not T_eq WHILE INT CHAR FLOAT VOID H MAINTOK INCLUDE BREAK CONTINUE IF ELSE PRINTF STRING FOR T_ques T_colon
...
POSTFIX_EXP
	: LIT {$$=$1;}
	| LIT T_incr {$$=buildTree("+",$1,1);}
	| LIT T_decr {$$=buildTree("-",$1,1);}
	;

LOOPS
      : WHILE '(' COND ')' LOOPBODY {$$=buildTree("WHILE",$3,$5);}
      | FOR '(' ASSIGN_EXPR ';' COND ';' POSTFIX_EXP  ')' LOOPBODY {$$=buildTree("FOR",$5,$9);} 
